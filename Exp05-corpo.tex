\titulo{PROJETO E VERIFICAÇÃO EXPERIMENTAL DE UM CONVERSOR BOOST} % Titulo em português
\title{DESIGN AND EXPERIMENTATION OF A BOOST CONVERTER} % Título em inglês

\maketitle

\editorfootnote{Artigo compilado em {\today} às {\currenttime}h, referente ao experimento de número 05 da disciplina de Laboratório de Eletrônica de Potência -- ET76C, ministrada pelo Prof. Dr. Adriano Ruseler, Eng.\\
...
Detalhar o projeto do conversor Boost (ver Planilhas \cite{noauthor_ptc_nodate}).
...
A \tabref{tab:IndutorBoost} apresenta os parâmetros de projeto do conversor Boost.
\begin{table}[!ht]
	\centering
%\begin{center}
	\captionof{table}{Parâmetros do indutor projetado para o conversor Boost}
...
A \tabref{tab:componentesBoost} apresenta a lista de componentes utilizados...
...
\bibliography{Refs/EXP05} % Inclui arquivos de referência
